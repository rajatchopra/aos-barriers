...
void gtmp_init(int num_threads){
  ...
  // rchopra33: Each flag should be in a different cache line to avoid
  // false sharing
  // This results in ensuring that the cache is not invalidated because of write on another node of the tree
  // Cache hit/miss has a 100x cost to it (for a subsequent hit) - so the performance will increaes manifold
  // when lots of barriers are present
  for (int j =0; j < num_nodes; j++) {
        posix_memalign((void **) &(nodes[j]), LEVEL1_DCACHE_LINESIZE, sizeof(node_t));
  }
